<section class="p-5 bg-primary pt-15 lg:p-15">
    <p class=" text-tertiary mb-2">HOW IT WORKS</p>
    <h2 class="text-4xl lg:text-7xl  text-white mb-10">What to expect on the day</h2>


    <div class="relative md:grid md:grid-cols-[repeat(4,auto)] md:justify-center md:gap-10">

        <div class="absolute left-5.5 top-0 bottom-0 w-0.5 bg-pastel/60 md:hidden"></div>

        <div class="flex items-start relative z-10 mb-10 md:flex-col  md:p-5">
            <div class="w-12 h-12 shrink-0 rounded-full bg-pastel flex items-center justify-center md:hidden">
                <span class="text-2xl text-black font-bodoni italic md:text-white/30 ">1</span>

            </div>
            <div class="flex flex-col ml-5 ">
                <span class="hidden text-2xl text-black font-bodoni italic md:text-white/30 md:text-8xl md:block"></span>
                <span class="relative hidden md:flex justify-center items-center bg-pastel w-10 h-10 md:mb-5 rounded-full"><span
                        class="hidden absolute md:flex md:text-3xl md:text-black  ">1</span></span>
                <h3 class="text-2xl mb-5 text-white md:text-3xl">Introduction & Safety</h3>
                <p class=" text-tertiary">A brief walkthrough of the facility, machine protocols, and safety procedures.
                </p>
            </div>
        </div>

        <div class="flex items-start relative z-10 mb-10 md:flex-col md:p-5 ">
            <div class="w-12 h-12 shrink-0 rounded-full bg-pastel flex items-center justify-center md:hidden">
                <span class="text-2xl text-black font-bodoni italic md:text-white/30 ">2</span>

            </div>
            <div class="flex flex-col ml-5 ">
                <span
                    class="hidden text-2xl text-black font-bodoni italic md:text-white/30 md:text-8xl md:block"></span>
                <span class="relative hidden md:flex justify-center items-center bg-pastel w-10 h-10 md:mb-5 rounded-full"><span
                        class="hidden absolute md:flex md:text-3xl md:text-black  ">2</span></span>
                <h3 class="text-2xl mb-5 text-white md:text-3xl">Profile Development</h3>
                <p class=" text-tertiary">We help you set your roast curves based on your green bean origins.</p>
            </div>
        </div>

        <div class="flex items-start relative z-10 mb-10 md:flex-col  md:p-5 ">
            <div class="w-12 h-12 shrink-0 rounded-full bg-pastel flex items-center justify-center md:hidden">
                <span class="text-2xl text-black font-bodoni italic md:text-white/30 ">3</span>

            </div>
            <div class="flex flex-col ml-5 ">
                <span
                    class="hidden text-2xl text-black font-bodoni italic md:text-white/30 md:text-8xl md:block"></span>
                <span class="relative hidden md:flex justify-center items-center bg-pastel w-10 h-10 md:mb-5 rounded-full"><span
                        class="hidden absolute md:flex md:text-3xl md:text-black  ">3</span></span>
                <h3 class="text-2xl mb-5 text-white md:text-3xl">Roasting Session</h3>
                <p class=" text-tertiary">The main event. You roast your beans under our watchful guidance.
            </div>
        </div>
        <div class="flex items-start relative z-10 mb-10 md:flex-col  md:p-5 ">
            <div class="w-12 h-12 shrink-0 rounded-full bg-pastel flex items-center justify-center md:hidden">
                <span class="text-2xl text-black font-bodoni italic md:text-white/30 ">4</span>

            </div>
            <div class="flex flex-col ml-5 ">
                <span
                    class="hidden text-2xl text-black font-bodoni italic md:text-white/30 md:text-8xl md:block"></span>
                <span class="relative hidden md:flex justify-center items-center bg-pastel w-10 h-10 md:mb-5 rounded-full"><span
                        class="hidden absolute md:flex md:text-3xl md:text-black  ">4</span></span>
                <h3 class="text-2xl mb-5 text-white  md:text-3xl">Packing</h3>
                <p class=" text-tertiary">Cooling, de-stoning, and weighing your beans for final packaging.
            </div>
        </div>
</section>
